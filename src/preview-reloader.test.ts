...
describe("local preview reload", () => {
  it("versions both auto-theme imports and scripts without changing source templates, and publishes only when ready", () => {
    ...
    mkdirSync(join(root, "dist/forgejo/public/assets/css"), { recursive: true });
    mkdirSync(join(root, "dist/forgejo/templates", "repo"), { recursive: true });
    const template = '<script src="{{AssetUrlPrefix}}/js/diff-view.js" defer></script>';
    writeFileSync(join(root, "dist/forgejo/templates", "repo", "commit.tmpl"), template);
    writeFileSync(
      join(root, "dist/forgejo/public/assets/css", "theme-github-auto.css"),
    ...
    reload.prepare();
    expect(existsSync(join(root, "dist/forgejo/public/assets/css", "preview-revision.json"))).toBe(false);
    reload.publish();
    const first = JSON.parse(
      readFileSync(join(root, "dist/forgejo/public/assets/css", "preview-revision.json"), "utf8")
    ).revision;
    const css = readFileSync(join(root, "dist/forgejo/public/assets/css", "theme-github-auto.css"), "utf8");
    expect(css).toContain(`theme-github-dark.css?preview=${first}`);
    expect(css).toContain(`theme-github-light.css?preview=${first}`);
    expect(readFileSync(join(root, "dist/forgejo/templates", "repo", "commit.tmpl"), "utf8")).toBe(template);
    ...
    reload.prepare();
    expect(
      JSON.parse(readFileSync(join(root, "dist/forgejo/public/assets/css", "preview-revision.json"), "utf8")).revision
    ).toBe(first);
    reload.publish();
    ...
    expect(readFileSync(join(root, "dist/forgejo/public/assets/css", "theme-github-auto.css"), "utf8")).not.toContain(
      first
    );
  });
});
